...
function finance_calc_1() {
  ...
  document.getElementById("front_finance_1_calc").style.backgroundColor =
    hexCode;
  document.querySelectorAll(
    ".Finance_1_radio_1_checkmark"
...
let calculate = () => {
  ...
  Finance_1_result.innerHTML = `<div>Principal Amount: <span><i class="fa-solid fa-indian-rupee-sign"></i>${p.toFixed(
    2
  )}</span></div>
<div>Total Compound Interest: <span><i class="fa-solid fa-indian-rupee-sign"></i>${compoundInterest.toFixed(
    2
  )}</span></div>
<div>Total Amount: <span><i class="fa-solid fa-indian-rupee-sign"></i>${amount.toFixed(
    2
  )}</span></div>`;
...
principal.addEventListener("click", function () {
  ...
  button_finance_1_calc.addEventListener("click", function () {
    if (duration == "year") {
      p_result =
        p_input.value / Math.pow(1 + r_input.value / 100, t_input.value);
    } else if (duration == "month") {
      p_result =
        p_input.value / Math.pow(1 + r_input.value / 100, t_input.value / 12);
    } else if (duration == "day") {
      p_result =
        p_input.value / Math.pow(1 + r_input.value / 100, t_input.value / 365);
    }
    button_finance_1_calc.style.bottom = "8vh";
    result.innerHTML = `<div>Principal Amount: <span><i class="fa-solid fa-indian-rupee-sign"></i>${p_result.toFixed(
      2
    )}</span></div>`;
  });
});
rate.addEventListener("click", function () {
  ...
  button_finance_1_calc.addEventListener("click", function () {
    if (duration == "year") {
      ...
      power_even = power % 2;
      if (power_even == 1 || base < 0) base = -base;
    ...
    button_finance_1_calc.style.bottom = "8vh";
    result.innerHTML = `<div>Rate of Interest: <span>${parseInt(
      r_result
    )}%</span></div>`;
  });
});
time.addEventListener("click", function () {
  ...
  button_finance_1_calc.addEventListener("click", function () {
    if (duration == "Yearly") {
      t_result =
        Math.log(p_input.value / r_input.value) /
        Math.log(1 + t_input.value / 100 / 1);
      result.innerHTML = `<div>Time: <span>${Math.round(
        t_result
      )} Years</span></div>`;
    } else if (duration == "Monthly") {
      t_result =
        Math.log(p_input.value / r_input.value) /
        (12)[Math.log(1 + t_input.value / 100 / 12)];
      result.innerHTML = `<div>Time: <span>${Math.round(
        t_result
      )} Months</span></div>`;
      ...
      t_result =
        Math.log(p_input.value / r_input.value) /
        (365)[Math.log(1 + t_input.value / 100 / 365)];
      result.innerHTML = `<div>Time: <span>${Math.round(
        t_result
      )} Days</span></div>`;
    }
